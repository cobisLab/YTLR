...
from utils import save_checkpoint, save_metrics, load_checkpoint, load_fine_tune_models
...
class Trainer:
    def __init__(self, name, train_loaders, 
                 device, cv, model_num, lr, epochs, warm_up_epoch, target_epochs, final_epochs, last_lr, metrics_dir, models_dir, now):
        self._cv = cv
        self._lr = lr
        self._name = name
    ...
    def run_epochs(self, model, train_loaders_in_cv, val_loader_in_cv, cv_num):
        ...
        optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=self._lr)
        total_steps = sum(len(loader) for loader in train_loaders_in_cv) * self._epochs
        #print('total_steps:',total_steps)
        scheduler = get_cosine_schedule_with_warmup(optimizer, 
                                                    num_warmup_steps=(total_steps/self._epochs)*self.warm_up_epoch, 
                                                    num_training_steps=total_steps,
                                                    num_target_steps=(total_steps/self._epochs)*self._target_epochs,
                                                    num_final_steps=(total_steps/self._epochs)*self._final_epochs,
        ...
        for epoch in range(self._epochs):
            ...
            print('LR = ' + str(optimizer.param_groups[0]["lr"])) # check lr
            # evaluation
            ...
            if val_loss < lowest_val_loss:
                ...
                save_path = self._models_dir+'model_'+str(self._model_num)+'_cv_' + str(cv_num) +'_'+self.now+'.pt'
    ...
    def fit_cv(self):
        ...
        # save 5-fold metrics
        save_path = self._metrics_dir+'model_'+ str(self._model_num) +'_cv_metrics_'+self.now+'.pt'
...
class BertTrainer(Trainer):
    def __init__(self, num_labels, premodel_dir, predevice,  freeze_embedding = True, freeze_layer_count = None, **kwargs):
        super(BertTrainer, self).__init__(**kwargs)
        self._num_labels = num_labels
        self._premodel_dir = premodel_dir
        self._predevice = predevice
    ...
    def _init_models(self):
        #where  = self._models_dir.split('/')[1]
        #init_model = BertClassifier.from_pretrained("dmis-lab/biobert-base-cased-v1.1",
        #                                      num_labels=self._num_labels,
        #                                      output_hidden_states=True)
        #models = [load_checkpoint('./data/'+where+'/model_4.pt', init_model, self._device)
        #        for _ in range(self._cv)]
    
        models = [BertClassifier.from_pretrained("dmis-lab/biobert-base-cased-v1.1",
                   num_labels=self._num_labels, output_hidden_states=True, hidden_dropout_prob=0.2, attention_probs_dropout_prob=0.2)
                   for _ in range(self._cv)]

        models = load_fine_tune_models(models, self._premodel_dir, 4, self._predevice)
        
        for model in models:
            if self.freeze_embedding:
    	        for param in model.bert.embeddings.parameters():
                    param.requires_grad = False
            if self.freeze_layer_count!=None:
                for layer in model.bert.encoder.layer[:self.freeze_layer_count]:
    ...
    '''
    def _init_model(self):
        where  = self._premodel_dir.split('/')[2]
        init_model = BertClassifier.from_pretrained("dmis-lab/biobert-base-cased-v1.1",
                                              num_labels=self._num_labels,
                                              output_hidden_states=True, hidden_dropout_prob=0.2, attention_probs_dropout_prob=0.2)
        #model = load_checkpoint('dataset/'+where+'/model_4.pt', init_model, self._device)
        model = load_checkpoint('model/indirect/model_1_cv_2_2022-02-10_15:48:05.811244.pt', init_model, self._device)
        if self.freeze_embedding:
            for param in model.bert.embeddings.parameters():
                param.requires_grad = False
        if self.freeze_layer_count!=None:
    ...
    def evaluate_batch(self, model, seqs, labels, masks, segments, sample_weight):
        batch_loss = 0.0
        step_count = 0
    ...
    def train(self, model, data_loaders, optimizer, scheduler):
        ...
        #for train_loader in data_loaders:
            ...
            #for data in train_loader:
                seqs, labels, masks, segments, sample_weight = [t.to(self._device) for t in data]
                ...
                batch_loss, batch_logits, batch_labels = self.evaluate_batch(model, seqs, labels, masks, segments, sample_weight)
...
class EnsembleTrainer(Trainer):
    def __init__(self, hidden_layer_num, input_shape, hidden, 
                 output_shape, dropout_rate, **kwargs):
        super(EnsembleTrainer, self).__init__(**kwargs)
        self._hidden_layer_num = hidden_layer_num
    ...
    def _init_models(self):
        models = [EnsembleNN(self._hidden_layer_num, self._input_shape, self._hidden, 
                             self._output_shape, self._dropout_rate, self._device) 
                             for _ in range(self._cv)]
        return models

    def _init_model(self):
        return EnsembleNN(self._hidden_layer_num, self._input_shape, self._hidden, 
                          self._output_shape, self._dropout_rate, self._device)

    def evaluate_batch(self, model, probs, labels):
        batch_loss = 0.0
        step_count = 0
        total_labels, total_logits = None, None
        ...
        with torch.no_grad():
            #for loader in data_loaders:
            #    for data in loader:
                    #probs, labels = [t.to(self._device) for t in data]
    ...
    def train(self, model, data_loaders, optimizer, scheduler):
        ...
        for train_loader in data_loaders:
            for data in train_loader:
                ...
                batch_loss, batch_logits, batch_labels = self.evaluate_batch(model, probs, labels)
                eval_batch_loss += batch_loss
    ...
    def evaluate(self, model, data_loaders, _dir ,cv_num, is_val=True):
        eval_loss = 0.0
        step_count = 0
        total_labels, total_logits = None, None
        ...
        with torch.no_grad():
            for loader in data_loaders:
                for data in loader:
                    ...
                    # 紀錄當前 batch loss
                    print(step_count, loss.item())
                    f1.write(str(logits)+','+str(+loss.item())+'\n')
                    eval_loss += loss.item()
                    step_count += 1
        eval_loss = eval_loss / step_count
